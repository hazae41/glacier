# Comparison with swr and react-query

## Prefetching

### xswr

```typescript
user.fetch()
```

### swr

```typescript
mutate('/api/user', fetch('/api/user').then(res => res.json()))
```

### react-query

```typescript
await queryClient.prefetchQuery(['user'], fetchUser)
```

## Optimistic updates

### xswr

```typescript
todos.update(old => [...old, todo])
```

### swr

```typescript
mutate('/api/todos', updateFn(user), { optimisticData: user, rollbackOnError: true })
```

### react-query

```typescript
useMutation(updateTodo, {
  onMutate: async newTodo => {
    await queryClient.cancelQueries(['todos'])
    const previousTodos = queryClient.getQueryData(['todos'])
    queryClient.setQueryData(['todos'], old => [...old, newTodo])
    return { previousTodos }
  },
  onError: (err, newTodo, context) => {
    queryClient.setQueryData(['todos'], context.previousTodos)
  },
  onSettled: () => {
    queryClient.invalidateQueries(['todos'])
  },
})
```

## Cancellation

### xswr

```typescript
user.aborter.abort()
```

### swr

Unsupported.

### react-query

```typescript
queryClient.cancelQueries(['todos'])
```

## Garbage collection

### xswr

Per-fetch expiration time. You can use response headers like `Cache-Control` to set an expiration time.

### swr

Unsupported.

### react-query

Global or per-query expiration time. You must define an expiration time at global scope or query scope.